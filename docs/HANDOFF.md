...
- 145 条家庭关系；
- 82 条史料；
- 首次进入前端即加载全部 270 人、145 条关系和 82 条相关史料；
- 展示阵营计数：魏 110、蜀 58、吴 68、其他 34；
...
`Person.importBatch` 只允许 `1 | 2 | 3 | 4`，人物档案分别显示第一至第四批
导入；校验器拒绝其他编号。
...
本批新增 35 人、35 条家庭关系和 3 条关系级史料。关系仅限既有六类：
...
不增加战争、敌对、主从、官职、联盟、事件或阵营连线。第四批每位新增人物只以
...
扩展的控制距离搜索上限已同步提高，保证 270 人图正式边继续绕开无关节点。
...
本次交付实测：lint 通过，69／69 测试通过，正式数据为 270／145／82，候选
数据仍为 99／738 且四个哈希一致，生产构建、离线构建和离线校验均通过。
1366×768 桌面浏览器确认第四批人物可搜索、人物档案和亲属关系显示正确；
390×844 窄屏地图宽度约 345px。两种尺寸均无横向页面溢出，控制台无
warning/error。

...
- 259 条家庭关系；
- 136 条史料；
- 281 人至少拥有一条正式关系，256 人完全孤立，覆盖率 52.3%；
- 展示阵营：魏 255、蜀 119、吴 124、其他 39；
- 首次进入前端即加载全部 537 人和 259 条关系；
...
8 条蜀系与其他关系；第六批名册首轮再新增 24 条卷二十宗室父子关系。当前正式关系
为 259 条。关系 JSON 序列化后的 SHA-256 为：

```text
a3343d7a060fce463a102748e4c062186255ddcdbcffdb02f3a01b53216a10d2
```
...
本轮实测：lint 通过，正式数据为 537／259／136；关系覆盖回归校验通过，基线为
281 人有关系、256 人孤立。首页完整图谱当前引用 133 条史料。候选数据仍为
99／738 且四个哈希一致；生产构建、临时目录离线构建和离线内联检查均通过。曹氏
最大连通分量扩展到 85 人后，布局仍通过确定性、节点无重叠和边不穿越无关人物测试。

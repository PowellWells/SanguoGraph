...
- 110 条家庭关系；
- 79 条史料；
- 首次进入前端即加载全部 235 人、110 条关系和 79 条相关史料；
- 展示阵营计数：魏 75、蜀 58、吴 68、其他 34；
...
`Person.importBatch` 只允许 `1 | 2 | 3`，人物档案分别显示第一、第二、第三批
导入；校验器拒绝其他编号。
...
本批新增 35 人、77 条家庭关系和 13 条关系级史料。关系仅限既有六类：
...
不增加战争、敌对、主从、官职、联盟、事件或阵营连线。此前“33 条关系冻结”
已经被用户在 2026-08-02 对第三批家庭扩展的明确授权取代；授权范围只覆盖家庭
关系，不得据此扩展其他类型。
...
- 曲线按 36px 逐级尝试两侧和多个控制点权重，首个无节点碰撞级别即停止；
...
0.8`，并在找到无穿节点路线后提前停止，避免 235 人图初始化变慢。
...
本次交付实测：lint 通过，67／67 测试通过，正式数据为 235／110／79，候选
数据为 99／738 且四个哈希一致，生产构建、离线构建和离线校验均通过。桌面
浏览器确认第三批搜索居中、文学层 110↔109 开关、吕布—貂蝉虚线及关系档案，

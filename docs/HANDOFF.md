...
更新时间：2026-08-12
当前分支：`codex/global-data-evidence-audit`

## 0. 新窗口接续说明（必须先读）

Round 5“全局遗漏、身份、来源和证据审计”已经完成实现，并继续坚持 577 人冻结、
六类家庭关系边界和证据优先：

- 新增何进—何晏、张既—张缉、应玚—应璩、步夫人—步骘 4 条《三国志》正文关系，
  另为原有孙和—孙皓父子关系补入关系级正文；
- 4 条新增关系均为 `confirmed + verified + direct_record`，共新增 5 条带原文摘录、
  URL 为空的本地史料；
- 修正潘夫人误挂“大懿皇后”别名；该称号只保留在琅邪王夫人条目；
- 为 6 条既有家族史料补齐原文，使全部 353 条正式关系都至少引用一条非空引文；
- 数据校验器新增 `RELATION_WITHOUT_QUOTED_SOURCE`，今后无关系级文本证据会直接失败；
- 人物维持 577，正式关系增至 353，史料增至 188；
- 371 人有关系、206 人孤立，覆盖率 64.3%；主要人物队列降至 61 人，第六批孤立人物
  降至 144 人；
- 全局类型为父亲 196、母亲 54、婚姻 50、养父 6、养母 2、宗族／姻亲 45；
- 关系 JSON SHA-256 为
  `95852ac6fe398417f448f99040db89a0bdbc81d65fd3f32e826146a2cdc0b962`。

Round 5 净连接 6 人，第一阶段距离 377 人入网／不超过 200 人孤立仍差 6 人；不通过
猜测关系、非亲属边或降低证据标准填充。下一轮为 Round 6：数据冻结、桌面与移动端
验收和离线发布验收。详细审计记录见 `docs/GLOBAL_DATA_EVIDENCE_AUDIT.md`。
...
- 577 个人物；
- 353 条家庭关系；
- 188 条史料，其中 185 条被人物或关系使用并可浏览；
- 371 人至少拥有一条正式关系，206 人完全孤立，覆盖率 64.3%；
- 展示阵营：魏 283、蜀 122、吴 133、其他 39；
- 首次进入前端即加载全部 577 人和 353 条关系；
...
4 条正文直接关系并净连接 6 人。当前正式关系为 353 条。
...
Round 5 完整交付验证已于 2026-08-12 完成：

- `npm run lint` 通过；
- `npm run test` 通过，共 26 个测试文件、129 项测试；
- 数据校验通过：577 人、353 条正式关系、188 条史料；
- 关系覆盖回归通过：371 人入网、206 人孤立、覆盖率 64.3%；
- processed 数据校验通过：99 人、738 条未核验候选、4 个文件哈希一致；
- 生产构建与离线单文件构建通过，候选数据不生成网页分块，`offline/index.html`
  为 868.6 KiB；
- 根入口与离线包校验通过；
- `npm audit --omit=dev` 为 0 个漏洞；
- 1366×768 桌面验收显示 577 人、353 条关系、185 个可浏览来源；主页和史料页均无
  维基体系可见文案或外链，史料卷次和引用仍正常显示；

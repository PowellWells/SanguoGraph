...
and `majorSources.ts`, and the third family batch is maintained in
`familyPersons.ts`, `familyRelations.ts`, and `familySources.ts`. Strict
...
| `importBatch` | Project ingestion batch: `1` for the original 24 people, `2` for the 176-person major-roster expansion, `3` for the 35-person family expansion |

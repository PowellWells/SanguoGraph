...
저장소 루트의 [`index.html`](index.html)을 두 번 클릭하면 로컬 브라우저에서 전체 그래프를 사용할 수 있습니다. 이 파일은 자동으로 [`offline/index.html`](offline/index.html)을 엽니다. 오프라인 파일에는 스타일, 애플리케이션 코드, 후보 데이터가 포함되어 있으므로 Node.js, 로컬 서버 또는 인터넷 연결이 필요하지 않습니다.
...
- 6개 가져오기 단계에 걸친 로컬 식별 인물 537명(`person:sg:*`).
- 정치 또는 전쟁 간선을 포함하지 않는 부·모·배우자·양친·종족 관계 184건.
- 첫 진입 시 정식 인물 537명을 모두 프런트엔드 지도에 불러오며, 현재 범위의 관계가 없는 인물도 독립 노드로 표시.
...
- 기본적으로 숨겨진 지연 로드 Wikidata 후보 레이어.
- 데스크톱과 모바일에 대응하는 레이아웃.

Wikidata QID는 외부 식별자일 뿐 프로젝트의 기본 키가 아니며, `confirmed` 관계를 성립시키는 근거가 될 수 없습니다.
...
커밋된 `data/processed` 레이어에는 99명과 Wikidata에서 파생된 미검증 후보 관계 738건이 들어 있습니다. 일반 웹 빌드는 독자가 스위치를 활성화할 때만 후보 모듈을 불러옵니다. 단일 파일 오프라인 빌드도 같은 후보 데이터를 포함하지만 기본적으로 숨깁니다. 두 빌드 모두 정식 인물 537명을 적용 경계로 사용하며, 현재 후보 스냅샷은 그중 일부만 다룹니다.
